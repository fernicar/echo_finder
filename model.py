...
class WorkerSignals(QObject):
    ...
    - status: str message
    - max_len_available: int
    """
    finished = Signal()
    error = Signal(tuple)
    result = Signal(list)
    status = Signal(str)
    max_len_available = Signal(int)
...
class EchoFinderWorker(QRunnable):
    ...
    """
    def __init__(self, text, min_len, max_len, whitelist):
        super().__init__()
    ...
    def run(self):
        try:
            ...
            # 1. Preprocessing and tokenizing with index tracking
            ...
            max_available_len = len(processed_tokens)
            self.signals.max_len_available.emit(max_available_len)
            
            if len(processed_tokens) < self.min_len:
                self.signals.result.emit([])
                self.signals.status.emit("Not enough words to find echoes.")
                return

            # 2. Generate n-grams and count frequencies
            self.signals.status.emit(f"Processing: Finding phrases (length {self.min_len}-{self.max_len})...")
            
            phrase_occurrences = defaultdict(list)
            for n in range(self.min_len, min(self.max_len, max_available_len) + 1):
            ...
            # Filter for phrases that are echoes (count >= 2)
            echoes = {
                phrase: occurrences
            ...
            # Sort echoes by length descending to prioritize longer matches
            sorted_echo_phrases = sorted(echoes.keys(), key=len, reverse=True)
    ...
    def _preprocess_text(self):
        """
        Converts text to lowercase, strips punctuation while respecting a whitelist,
        and returns a list of token dictionaries with original indices.
        """
        tokens = []
        # Regex to find words or whitelisted abbreviations
        ...
        for match in re.finditer(word_pattern, self.text, re.IGNORECASE):
            ...
            # Special handling for whitelisted items to keep their original form's casing for matching
...
class ProjectModel(QObject):
    ...
    def load_project(self, filepath: str):
        try:
            ...
            self.status_message.emit(f"Error: File not found at {filepath}")
        except json.JSONDecodeError:
            self.status_message.emit(f"Error: Could not parse project file. Invalid JSON format.")
        except Exception as e:
    ...
    def save_project(self, filepath: str):
        try:
            ...
            self.status_message.emit("Error: Permission denied when saving.")
        except Exception as e:
    ...
    def process_text(self):
        ...
        min_len = self.data.get("min_phrase_length", 2)
        max_len = self.data.get("max_phrase_length", 8)
        whitelist = self.data.get("custom_whitelist", [])
        
        worker = EchoFinderWorker(text, min_len, max_len, whitelist)
        worker.signals.result.connect(self._on_processing_result)
        worker.signals.status.connect(self.status_message)
        worker.signals.max_len_available.connect(self.max_len_available)
    ...
    def _on_processing_result(self, results):
        ...
        self.sort_results() # Apply current sort preset
    ...
    def sort_results(self):
        ...
        if preset == "most_repeated_short_to_long":
            # Sort by count (desc), then length (asc), then phrase (asc)
            results.sort(key=lambda x: (-x['count'], x['length'], x['phrase']))
        elif preset == "longest_first_by_word_count":
            # Sort by length (desc), then count (asc), then phrase (asc)
            results.sort(key=lambda x: (-x['length'], x['count'], x['phrase']))
