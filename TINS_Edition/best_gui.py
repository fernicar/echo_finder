...
class MainWindow(QMainWindow):
    ...
    def _init_ui(self):
        ...
        select_all_action = QAction("Select &All", self) # QAction for select all
        ...
        self.story_display.textChanged.connect(self._update_counts) # Connect signal
        ...
        outline_layout = QVBoxLayout(outline_tab) # QVBoxLayout for outline tab
        self.outline_display = QTextEdit() # QTextEdit for outline display
        self.outline_display.setPlaceholderText("Enter your story outline here...")
        ...
        self.chat_preview_display.setPlaceholderText("Chatlog will appear here...")
        ...
        self.canon_display.setPlaceholderText("Enter canon details, established facts, and continuity notes here...")
        ...
        self.world_setting_display.setPlaceholderText("Describe the world, its rules, locations, and overall atmosphere...")
        ...
        self.character_profiles_display.setPlaceholderText("Manage character sheets, backstories, relationships, and arcs...")
        character_profiles_layout.addWidget(self.character_profiles_display)
        self.monitor_tabs.addTab(character_profiles_tab_widget, "Character Profiles")

        # Checklist Tab
        checklist_tab_widget = QWidget()
        checklist_layout = QVBoxLayout(checklist_tab_widget)
        self.checklist_widget = QListWidget()
        self.checklist_widget.setSelectionMode(QListWidget.SelectionMode.ExtendedSelection) # Allow multi-select
        ...
        self.secondary_input.setPlaceholderText("Define system prompt or instructions here...")
...
"""
    ...
    def _handle_send_chat_message(self):
        ...
        if text:
            ...
            QTimer.singleShot(500, lambda: self.chat_preview_display.append(f"Bot: Acknowledged: '{text}'\n"))
    ...
    def _handle_find(self):
        ...
        search_term, ok = QInputDialog.getText(self, "Find Text", "Enter text to find:")
        if ok and search_term:
            self.current_search_term = search_term
            self._find_text_in_story_display(self.current_search_term, find_next=False)
        elif ok and not search_term: # User entered blank search
             QMessageBox.information(self, "Find Text", "Search term cannot be empty.")
    ...
    def _handle_find_next(self):
        ...
        if not self.current_search_term:
            ...
            self.find_text_in_story_display_with_options(self.current_search_term, find_next=True)
    ...
    def find_text_in_story_display_with_options(self, search_term, find_next=False, case_sensitive=False, whole_words=False):
        ...
        find_flags = QTextDocument.FindFlag(0) # PySide6 uses QTextDocument.FindFlags()
        if case_sensitive:
            ...
            if find_next: # Only show wrap-around for "Find Next" type operations
                ...
                if reply == QMessageBox.StandardButton.Yes:
                    ...
                    if self.story_display.find(search_term, find_flags):
                        ...
                        QMessageBox.information(self, "Find Text", f"'{search_term}' not found in document.")
                        return False
    ...
    def _handle_replace_dialog(self):
        ...
        if self.replace_dialog is None:
            self.replace_dialog = ReplaceDialog(self) # Pass MainWindow instance

        # Pre-fill find text from current search term or last used in dialog
        last_find_text = self.settings.value("replaceDialog/findText", self.current_search_term)
        self.replace_dialog.find_edit.setText(last_find_text) # type: ignore

        last_replace_text = self.settings.value("replaceDialog/replaceText", "")
        self.replace_dialog.replace_edit.setText(last_replace_text) # type: ignore

        case_checked = self.settings.value("replaceDialog/caseSensitive", False, type=bool)
        self.replace_dialog.case_checkbox.setChecked(case_checked) # type: ignore

        words_checked = self.settings.value("replaceDialog/wholeWords", False, type=bool)
        self.replace_dialog.words_checkbox.setChecked(words_checked) # type: ignore
        ...
        if self.replace_dialog.exec(): # exec() returns QDialog.DialogCode.Accepted if Ok/Accept
            ...
            self.settings.setValue("replaceDialog/findText", self.replace_dialog.find_edit.text())
            self.settings.setValue("replaceDialog/replaceText", self.replace_dialog.replace_edit.text())
            self.settings.setValue("replaceDialog/caseSensitive", self.replace_dialog.case_checkbox.isChecked())
            self.settings.setValue("replaceDialog/wholeWords", self.replace_dialog.words_checkbox.isChecked())
        # else:
    ...
    def _handle_send(self):
        ...
        elif active_tab == self.input_tabs.widget(1):  # System Prompt Input tab
...
class ReplaceDialog(QDialog):
    def __init__(self, parent_main_window: MainWindow):
        ...
        # Connections
        self.cancel_button.clicked.connect(self.reject) # QDialog.reject() for cancel
    ...
    def _on_find_next(self):
        ...
        if not term:
            QMessageBox.information(self, "Find Next", "Please enter a search term.")
            return
    ...
    def _on_replace(self):
        ...
        if not find_term:
            QMessageBox.information(self, "Replace", "Please enter a search term.")
            return
    ...
    def _on_replace_all(self):
        ...
        if not find_term:
            QMessageBox.information(self, "Replace All", "Please enter a search term.")
            return
        ...
        while story_display.find(find_term, find_flags): # find() selects the found text and moves cursor
            story_display.textCursor().insertText(replace_term) # Replace current selection
            replacements_count += 1

        if replacements_count > 0:
            QMessageBox.information(self, "Replace All", f"Made {replacements_count} replacement(s).")
            # Store terms
            self.main_window.settings.setValue("replaceDialog/findText", find_term)
            self.main_window.settings.setValue("replaceDialog/replaceText", replace_term)
            self.main_window.settings.setValue("replaceDialog/caseSensitive", case_sensitive)
            self.main_window.settings.setValue("replaceDialog/wholeWords", whole_words)
        else:
            QMessageBox.information(self, "Replace All", f"'{find_term}' not found.")

    def accept(self): # Override accept to save settings if dialog is closed via "Enter" on a button etc.
        self.main_window.settings.setValue("replaceDialog/findText", self.find_edit.text())
        self.main_window.settings.setValue("replaceDialog/replaceText", self.replace_edit.text())
        self.main_window.settings.setValue("replaceDialog/caseSensitive", self.case_checkbox.isChecked())
        self.main_window.settings.setValue("replaceDialog/wholeWords", self.words_checkbox.isChecked())
        super().accept()
